...
function travelToPlanet(ref) {
    ...
    if (cometaVisual) cometaVisual.style.display = "none";
...
function generarCieloEstrellado() {
    ...
    // Redujimos un poco la cantidad. Con la aceleración, brillan mejor y no necesitas tantas.
    const numeroEstrellas = esMovil ? 40 : 100; 
    
    // El DocumentFragment es como una caja invisible. Metemos todo aquí primero.
    ...
    for (let i = 0; i < numeroEstrellas; i++) {
        ...
        const tamano = Math.random() * 1.5 + 0.5;
        ...
        // 🚀 OPTIMIZACIÓN CLAVE: Aceleración por GPU
        estrella.style.willChange = 'opacity, transform'; 
        estrella.style.animation = `parpadeo-estelar ${Math.random() * 3 + 2}s infinite alternate ease-in-out`;
        
        fragmento.appendChild(estrella);
    }
    
    // Inyectamos todas las estrellas juntas de un solo golpe. ¡Cero lag!
    galaxia.appendChild(fragmento);
}
